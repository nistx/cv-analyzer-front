...
function JobDescriptionUpload() {
  ...
  const { progress, currentCV, estimatedTimeRemaining, upload } = useJobUpload(jobDescription , files);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        console.log('Datos de cvs:', data.cvs);
        navigate('/ranking', { state: data });
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Error de red. Por favor, verifique su conexión.';
      toast({
        variant: 'destructive',
        title: 'Error',
        ...
        <div className="w-1/2 bg-black flex items-start">
          <div className='px-[20vh] text-white mt-[15vh]'>
            <p className="font-bold text-[40px] leading-[1] mb-10">
              Encuentra el candidato perfecto con <span className="text-[#8e8f9e]">IA</span>
            </p>
            <Alert>
